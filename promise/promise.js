class Promise {
  ...
  then(onFulfilled, onRejected) {
    ...
    const _promise =  new Promise((resolve, reject) => { // “箭头函数” 的 this，总是指向定义时所在的对象，而不是运行时所在的对象(注: 只有函数运行后，箭头函数才会按照定义生成)。也可以简单理解为 ——> 总是指向所在函数运行时的 this。
      // console.log(this === that) // this 指向前一个 promise。因为这里的 then() 是被前一个 promise 调用的，而当前 Promise 中的 fn(箭头函数) 的 this 指向 then，
      // then 被前一个 Promise 调用，所以会隐式地绑定到前一个 Promise 上，所以 this 指向前一个 promise。
      switch (this.status) {
        case 'pending':
      ...
      function realFulfilled(value) {
        if (!isFunction(onFulfilled)) {
          return resolve(value) // 实现 then() 穿透
        }
  ...
  hookNext(p, r, resolve, reject) {
    if (r === p) { // 不允许 then 中的 onFulfilled 返回自身的 promise
    ...
    try {
      if (isThenable(r)) {
        ...
        r.then(resolve, reject) // 让内部 promise 去执行外部 promise 的 resolve，并把 value 值传给外部 promise
      } else {
        // console.log('成功调用 then，现在我要调用新 promise 中的 resolve() 了')
        resolve(r) // 注意，前一个 promise 的处理结果不影响当前 promise 的状态，当前 promise 默认状态为成功
      }
